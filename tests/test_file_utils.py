...
from utils.file_utils import (parse_filename, convert_interval_to_integer,
                              ...
                              load_tables_in_path, list_tests, is_test,
                              is_sql_query, is_processor,
                              is_processor_select_query, has_processor,
                              list_files, load_select_query)
...
def test_is_processor_select_query(views_path):
    results = [is_processor_select_query(f)
               for f in filenames(views_path)]
...
def test_load_select_query(views_path):
    ...
    assert pytest.similar(countries,
                          '''select country, continent
                             from first.countries_raw;''')
...
def test_list_files(views_path):
    ...
    assert test_files == ['./views/first.countries_test.sql',
                          './views/first/cities_test.sql']

    python_processors = list_files(views_path,
                                   match=is_processor,
                                   mask='*.py')
