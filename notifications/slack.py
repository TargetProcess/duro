...
def delay_duplicates(timeout=10 * 60):
    def wrap(f):
        ...
        def wrapper(*args, **kwargs):
            key = "".join(str(arg) for arg in args + tuple(kwargs.values()))
...
@delay_duplicates()
def send_slack_notification(message: str, title: str = None, message_type: str = None):
...
def choose_channel_and_emoji(slack_config: SlackConfig, message_type: str) -> Tuple:

    channels = {
        "success": (slack_config.success_channel, ":white_check_mark:"),
        "failure": (slack_config.failure_channel, ":scream_cat:"),
        "log": (slack_config.log_channel, ":white_check_mark:"),
    }

    return channels.get(message_type, (slack_config.failure_channel, ":scream_cat:"))


def build_message(
    slack_config: SlackConfig, message, title=None, message_type=None
