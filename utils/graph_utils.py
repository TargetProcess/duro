...
def detect_cycles(source_graph: nx.DiGraph) -> Tuple[bool, List]:
    return nx.is_directed_acyclic_graph(source_graph), nx.simple_cycles(source_graph)


def copy_graph_without_attributes(
    source_graph: nx.DiGraph, attributes: List
) -> nx.DiGraph:
